...
var _MatSort = class _MatSort {
  ...
  constructor(_defaultOptions) {
    this._defaultOptions = _defaultOptions;
    this._initializedStream = new import_rxjs6.ReplaySubject(1);
...
var _MatSortHeaderIntl = class _MatSortHeaderIntl {
  constructor() {
    this.changes = new import_rxjs6.Subject();
  }
};
...
var _MatSortHeader = class _MatSortHeader {
  ...
  _handleStateChanges() {
    this._rerenderSubscription = (0, import_rxjs6.merge)(this._sort.sortChange, this._sort._stateChanges, this._intl.changes).subscribe(() => {
      if (this._isSorted()) {
        this._updateArrowDirection();
        if (this._viewState.toState === "hint" || this._viewState.toState === "active") {
          this._disableViewStateAnimation = true;
